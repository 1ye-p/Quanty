  ...
  return (
    <div className="grid grid-cols-5 gap-3">
      <div className="card p-3 text-center">
        <div className="text-xs text-gray-500 mb-1">{t('component.datasets.price_stats.latest_price')}</div>
        <div className="text-lg font-semibold">{stats.latest_price.toFixed(2)}</div>
      </div>
      <div className="card p-3 text-center">
        <div className="text-xs text-gray-500 mb-1">{t('component.datasets.price_stats.change_pct')}</div>
        <div className={`text-lg font-semibold ${changeColor}`}>
          {changeSign}{(stats.change_pct * 100).toFixed(2)}%
        </div>
      </div>
      <div className="card p-3 text-center">
        <div className="text-xs text-gray-500 mb-1">{t('component.datasets.price_stats.high_52w')}</div>
        <div className="text-lg font-semibold">{stats.high_52w.toFixed(2)}</div>
      </div>
      <div className="card p-3 text-center">
        <div className="text-xs text-gray-500 mb-1">{t('component.datasets.price_stats.low_52w')}</div>
        <div className="text-lg font-semibold">{stats.low_52w.toFixed(2)}</div>
      </div>
      <div className="card p-3 text-center">
        <div className="text-xs text-gray-500 mb-1">{t('component.datasets.price_stats.avg_volume')}</div>
