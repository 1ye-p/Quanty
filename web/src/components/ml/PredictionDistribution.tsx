...
export function PredictionDistribution({ bins }: PredictionDistributionProps) {
  ...
  if (!bins.length) {
    return (
      <div className="flex items-center justify-center h-48 text-gray-400 text-sm bg-gray-50 rounded-lg">
        {t('component.ml.prediction_distribution.empty')}
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-4">
      <h3 className="font-semibold text-gray-700 mb-2">{t('component.ml.prediction_distribution.title')}</h3>

      {stats && (
        <div className="flex items-center gap-4 mb-3 text-xs text-gray-500">
          <span>{t('component.ml.prediction_distribution.mean')}: <span className="font-mono text-gray-700">{formatNum(stats.mean)}</span></span>
          <span>{t('component.ml.prediction_distribution.std')}: <span className="font-mono text-gray-700">{formatNum(stats.std)}</span></span>
          <span>{t('component.ml.prediction_distribution.samples')}: <span className="font-mono text-gray-700">
        ...
        <BarChart data={chartData} margin={{ top: 4, right: 16, left: -10, bottom: 0 }}>
          ...
          <Tooltip
            labelFormatter={(_: string, payload: Array<{ payload?: { label?: string } }>) => {
              ...
              return label ? t('component.ml.prediction_distribution.tooltip_range', { range: label }) : ''
            }}
            formatter={(value: number) => [value.toLocaleString(), t('component.ml.prediction_distribution.tooltip_count')]}
          />
          {stats && (
            <ReferenceLine
              ...
              strokeDasharray="4 4"
              label={{
                value: t('component.ml.prediction_distribution.ref_mean'),
                position: 'top',
                fill: '#ef4444',
                fontSize: 10,
              }}
